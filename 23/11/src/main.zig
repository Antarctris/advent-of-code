...
pub fn solveChallenge(allocator: Allocator, input: []const u8, factor_a: u64, factor_b: u64) [2]u64 {
    ...
    var galaxies_one = std.ArrayList(grid.Vec2).init(allocator);
    defer galaxies_one.deinit();

    var galaxies_two = std.ArrayList(grid.Vec2).init(allocator);
    ...
    for (0..map.height) |y| {
        var empty: bool = true;
        for (0..map.width) |x| {
            var p = grid.Vec2{ .x = @intCast(x), .y = @intCast(y) };
            if (map.get(p).? == '#') {
                _ = empty_cols.fetchOrderedRemove(@intCast(x));
                empty = false;
                galaxies_one.append(p.translate(grid.Vec2{ .x = 0, .y = @intCast(empty_rows * (factor_a - 1)) })) catch unreachable;
                galaxies_two.append(p.translate(grid.Vec2{ .x = 0, .y = @intCast(empty_rows * (factor_b - 1)) })) catch unreachable;
            }
        }
        if (empty) {
            empty_rows += 1;
    ...
    while (empty_cols.popOrNull()) |col| {
        for (0..galaxies_one.items.len) |i| {
            if (galaxies_one.items[i].x > col.key and galaxies_one.items[i].x < last) {
                galaxies_one.items[i] = galaxies_one.items[i].translate(grid.Vec2{ .x = @intCast((empty_cols.keys().len + 1) * (factor_a - 1)), .y = 0 });
                galaxies_two.items[i] = galaxies_two.items[i].translate(grid.Vec2{ .x = @intCast((empty_cols.keys().len + 1) * (factor_b - 1)), .y = 0 });
            }
        }
