...
pub fn main() !void {
    ...
    for (builtin.test_functions) |t| {
        ...
        printer.print("Testing {s}: ", .{t.name});
        const result = t.func();

        if (std.testing.allocator_instance.deinit() == .leak) {
            leak += 1;
            printer.print_status(.fail, "\n{s}\n\"{s}\" - Memory Leak\n{s}\n", .{ BORDER, t.name, BORDER });
        }
    ...
    if (skip > 0) {
        printer.print_status(.skip, "{d} test{s} skipped\n", .{ skip, if (skip != 1) "s" else "" });
    }
    if (leak > 0) {
        printer.print_status(.fail, "{d} test{s} leaked\n", .{ leak, if (leak != 1) "s" else "" });
    }
...
const Printer = struct {
    ...
    fn print_status(self: Printer, s: Status, comptime format: []const u8, args: anytype) void {
