...
fn updateRecord(allocator: Allocator, record: ResultRecord) !void {
    if (std.fs.cwd().readFileAlloc(allocator, record_path, 1 << 16)) |old_file| { // 64KiB
        ...
        var result_records = std.ArrayList(ResultRecord).empty;
        defer result_records.deinit(allocator);
        try result_records.appendSlice(allocator, old_record.value.results);
        ...
        if (index < result_records.items.len and result_records.items[index].id == record.id) {
            ...
            try result_records.insert(allocator, index, record);
        }
        ...
        var record_file_writer = record_file.writer(&buffer);
        const record_writer: *std.io.Writer = &record_file_writer.interface;

        const formatter = std.json.fmt(new_record, .{ .whitespace = .indent_4 });
        try formatter.format(record_writer);
